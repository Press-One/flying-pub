...
export default observer(() => {
  ...
  const { userStore, readerWalletStore } = useStore();
  const mixinWalletClientId = userStore.user.SSO
    ? userStore.user.SSO.reader.mixinWalletClientId
    : userStore.user.mixinWalletClientId;
  const { isFetchedReceipts, receipts, receiptLimit, hasMoreReceipt } = readerWalletStore;
    ...
    (async () => {
      try {
        const receipts = await FinanceApi.getReceipts({
          offset: 0,
          limit: receiptLimit,
          filterType: readerWalletStore.filterType,
        });
        page.current += 1;
        await sleep(500);
        readerWalletStore.setReceipts(receipts);
        readerWalletStore.setIsFetchedReceipts(true);
      } catch (err) {}
    })();
  }, [readerWalletStore, receiptLimit]);
  ...
  const loadMore = async () => {
    try {
      ...
      page.current += 1;
      await sleep(500);
      readerWalletStore.addReceipts(newReceipts);
      setLoading(false);
    } catch (err) {}
  };
    ...
    <Fade in={true} timeout={500}>
      <div className="root">
        {receipts.map((receipt: any) => (
          <div key={receipt.id}>
            {Receipt(receipt, mixinWalletClientId, readerWalletStore.canSpendBalance || isPc)}
          </div>
        ))}
        {/* 用解决定位，解决移动端无法点击的问题 */}
        {hasMoreReceipt && (
        ...
        {receipts.length === 0 && (
          <div className="text-gray-500 pt-32 text-center text-sm">
            暂时没有{readerWalletStore.rewardOnly ? '打赏' : '交易'}记录
          </div>
        )}
        <style jsx>{`
          .root {
