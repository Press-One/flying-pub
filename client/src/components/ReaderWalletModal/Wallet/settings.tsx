...
export default observer(() => {
  ...
  }, [readerWalletStore, setStep]);

  const { isFetchedIsCustomPinExist, isCustomPinExist } = readerWalletStore;
      ...
      if (value.length === 6) {
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            onCloseModal();
            await sleep(400);
            readerWalletStore.setIsCustomPinExist(latestIsCustomPinExist);
            snackbarStore.show({
              message: '密码设置成功',
            });
          } catch (err) {
