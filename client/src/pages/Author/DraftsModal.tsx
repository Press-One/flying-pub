...
const DraftEntry = observer(
  (props: { file: IFile; refreshPosts: () => void; close: () => void }) => {
    const { file } = props;
...
const Drafts = observer((props: any) => {
  const state = useLocalStore(() => ({
    get hasFiles() {
  ...
  return (
    <div className="bg-white rounded-12 text-gray-4a">
      ...
      <div className="pb-8 h-80-vh overflow-y-auto">
        <div ref={infiniteRef}>
          {!state.isFetched && (
